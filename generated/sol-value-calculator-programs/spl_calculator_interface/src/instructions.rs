...
pub enum SplCalculatorProgramIx {
...
impl BorshSerialize for SplCalculatorProgramIx {
...
pub struct LstToSolAccounts<'me, 'info> {
    ///The SPL stake pool LST mint
    pub lst: &'me AccountInfo<'info>,
    ///The SplCalculatorState PDA
    pub state: &'me AccountInfo<'info>,
    ///The main stake pool account
    pub stake_pool: &'me AccountInfo<'info>,
    ///The spl stake pool program
    pub spl_program: &'me AccountInfo<'info>,
    ///The spl stake pool program executable data
    pub spl_program_data: &'me AccountInfo<'info>,
}
#[derive(Copy, Clone, Debug)]
...
impl From<&LstToSolAccounts<'_, '_>> for LstToSolKeys {
    fn from(accounts: &LstToSolAccounts) -> Self {
        Self {
            ...
            spl_program: *accounts.spl_program.key,
            spl_program_data: *accounts.spl_program_data.key,
        }
    }
}
impl From<&LstToSolKeys> for [AccountMeta; LST_TO_SOL_IX_ACCOUNTS_LEN] {
    fn from(keys: &LstToSolKeys) -> Self {
        [
            ...
            AccountMeta::new_readonly(keys.spl_program, false),
            AccountMeta::new_readonly(keys.spl_program_data, false),
        ]
    }
}
impl From<[Pubkey; LST_TO_SOL_IX_ACCOUNTS_LEN]> for LstToSolKeys {
    fn from(pubkeys: [Pubkey; LST_TO_SOL_IX_ACCOUNTS_LEN]) -> Self {
        Self {
            ...
            spl_program: pubkeys[3],
            spl_program_data: pubkeys[4],
        }
    }
}
    ...
    for [AccountInfo<'info>; LST_TO_SOL_IX_ACCOUNTS_LEN]
{
    fn from(accounts: &LstToSolAccounts<'_, 'info>) -> Self {
        [
            ...
            accounts.spl_program_data.clone(),
        ]
    }
}
...
pub fn lst_to_sol_verify_account_keys(
    ...
    for (actual, expected) in [
        ...
        (accounts.spl_program.key, &keys.spl_program),
        (accounts.spl_program_data.key, &keys.spl_program_data),
    ] {
        if actual != expected {
...
impl From<[Pubkey; UPDATE_LAST_UPGRADE_SLOT_IX_ACCOUNTS_LEN]> for UpdateLastUpgradeSlotKeys {
    fn from(pubkeys: [Pubkey; UPDATE_LAST_UPGRADE_SLOT_IX_ACCOUNTS_LEN]) -> Self {
        Self {
            manager: pubkeys[0],
            state: pubkeys[1],
            spl_program: pubkeys[2],
            spl_program_data: pubkeys[3],
        }
    }
}
