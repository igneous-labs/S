...
pub enum SoceanCalculatorProgramIx {
...
impl BorshSerialize for SoceanCalculatorProgramIx {
...
impl SoceanCalculatorProgramIx {
...
pub struct LstToSolAccounts<'me, 'info> {
    ...
    ///The SoceanCalculatorState PDA
    pub state: &'me AccountInfo<'info>,
    ///The main stake pool account
    pub stake_pool: &'me AccountInfo<'info>,
    ///The socean program
    pub socean_program: &'me AccountInfo<'info>,
    ///The socean program executable data
    pub socean_program_data: &'me AccountInfo<'info>,
}
#[derive(Copy, Clone, Debug)]
pub struct LstToSolKeys {
    ...
    ///The socean program
    pub socean_program: Pubkey,
    ///The socean program executable data
    pub socean_program_data: Pubkey,
}
impl From<&LstToSolAccounts<'_, '_>> for LstToSolKeys {
    fn from(accounts: &LstToSolAccounts) -> Self {
        Self {
            ...
            socean_program: *accounts.socean_program.key,
            socean_program_data: *accounts.socean_program_data.key,
        }
    }
}
impl From<&LstToSolKeys> for [AccountMeta; LST_TO_SOL_IX_ACCOUNTS_LEN] {
    fn from(keys: &LstToSolKeys) -> Self {
        [
            ...
            AccountMeta::new_readonly(keys.socean_program, false),
            AccountMeta::new_readonly(keys.socean_program_data, false),
        ]
    }
}
impl From<[Pubkey; LST_TO_SOL_IX_ACCOUNTS_LEN]> for LstToSolKeys {
    fn from(pubkeys: [Pubkey; LST_TO_SOL_IX_ACCOUNTS_LEN]) -> Self {
        Self {
            ...
            socean_program: pubkeys[3],
            socean_program_data: pubkeys[4],
        }
    }
}
    ...
    for [AccountInfo<'info>; LST_TO_SOL_IX_ACCOUNTS_LEN]
{
    fn from(accounts: &LstToSolAccounts<'_, 'info>) -> Self {
        [
            ...
            accounts.socean_program.clone(),
            accounts.socean_program_data.clone(),
        ]
    }
}
    ...
    for LstToSolAccounts<'me, 'info>
{
    fn from(arr: &'me [AccountInfo<'info>; LST_TO_SOL_IX_ACCOUNTS_LEN]) -> Self {
        Self {
            ...
            state: &arr[1],
            stake_pool: &arr[2],
            socean_program: &arr[3],
            socean_program_data: &arr[4],
        }
    }
}
...
pub fn lst_to_sol_verify_account_keys(
    ...
    for (actual, expected) in [
        ...
        (accounts.socean_program.key, &keys.socean_program),
        (accounts.socean_program_data.key, &keys.socean_program_data),
    ] {
        if actual != expected {
...
impl From<[Pubkey; UPDATE_LAST_UPGRADE_SLOT_IX_ACCOUNTS_LEN]> for UpdateLastUpgradeSlotKeys {
    fn from(pubkeys: [Pubkey; UPDATE_LAST_UPGRADE_SLOT_IX_ACCOUNTS_LEN]) -> Self {
        Self {
            manager: pubkeys[0],
            state: pubkeys[1],
            socean_program: pubkeys[2],
            socean_program_data: pubkeys[3],
        }
    }
}
    ...
    for UpdateLastUpgradeSlotAccounts<'me, 'info>
{
    fn from(arr: &'me [AccountInfo<'info>; UPDATE_LAST_UPGRADE_SLOT_IX_ACCOUNTS_LEN]) -> Self {
        Self {
            manager: &arr[0],
            state: &arr[1],
            socean_program: &arr[2],
            socean_program_data: &arr[3],
        }
    }
}
#[derive(BorshDeserialize, BorshSerialize, Clone, Debug, PartialEq)]
...
pub struct InitAccounts<'me, 'info> {
    ///The account paying for SoceanCalculatorState's rent
