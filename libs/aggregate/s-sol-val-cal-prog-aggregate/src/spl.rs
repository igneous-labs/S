...
use spl_calculator_lib::{
    deserialize_spl_stake_pool_checked, deserialize_stake_pool_checked, SplSolValCalc,
    SplStakePoolCalc,
};
...
impl LstSolValCalc for SplLstSolValCalc {
    ...
    fn ix_accounts(&self) -> Result<Vec<AccountMeta>, Box<dyn Error + Send + Sync>> {
        Ok(Vec::from(
            <[AccountMeta; generic_pool_calculator_interface::LST_TO_SOL_IX_ACCOUNTS_LEN]>::from(
                generic_pool_calculator_interface::LstToSolKeys::from(
...
impl TryFrom<KnownLstSolValCalc> for SplLstSolValCalc {
    ...
    fn try_from(value: KnownLstSolValCalc) -> Result<Self, Self::Error> {
        match value {
            ...
            // TODO: uncomment once we add more variants
