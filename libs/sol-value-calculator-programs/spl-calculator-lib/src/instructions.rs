...
pub fn spl_update_last_upgrade_slot_invoke<'info, A: Into<UpdateLastUpgradeSlotIxArgs>>(
    accounts: &UpdateLastUpgradeSlotAccounts<'_, 'info>,
    args: A,
) -> ProgramResult {
    let ix = spl_update_last_upgrade_slot_ix(accounts, args)?;
    let account_info: [AccountInfo<'info>; UPDATE_LAST_UPGRADE_SLOT_IX_ACCOUNTS_LEN] =
        accounts.into();
    invoke(&ix, &account_info)
}
pub fn spl_update_last_upgrade_slot_invoke_signed<'info, A: Into<UpdateLastUpgradeSlotIxArgs>>(
    accounts: &UpdateLastUpgradeSlotAccounts<'_, 'info>,
    args: A,
    seeds: &[&[&[u8]]],
) -> ProgramResult {
...
pub fn spl_set_manager_ix<K: Into<SetManagerKeys>, A: Into<SetManagerIxArgs>>(
    accounts: K,
    args: A,
) -> std::io::Result<Instruction> {
...
pub fn spl_set_manager_invoke<'info, A: Into<SetManagerIxArgs>>(
    accounts: &SetManagerAccounts<'_, 'info>,
    args: A,
) -> ProgramResult {
    let ix = spl_set_manager_ix(accounts, args)?;
    let account_info: [AccountInfo<'info>; SET_MANAGER_IX_ACCOUNTS_LEN] = accounts.into();
    invoke(&ix, &account_info)
}
pub fn spl_set_manager_invoke_signed<'info, A: Into<SetManagerIxArgs>>(
    accounts: &SetManagerAccounts<'_, 'info>,
    args: A,
    seeds: &[&[&[u8]]],
) -> ProgramResult {
...
pub fn spl_init_ix<K: Into<InitKeys>, A: Into<InitIxArgs>>(
    accounts: K,
    args: A,
) -> std::io::Result<Instruction> {
...
pub fn spl_init_invoke<'info, A: Into<InitIxArgs>>(
    accounts: &InitAccounts<'_, 'info>,
    args: A,
) -> ProgramResult {
    let ix = spl_init_ix(accounts, args)?;
    let account_info: [AccountInfo<'info>; INIT_IX_ACCOUNTS_LEN] = accounts.into();
    invoke(&ix, &account_info)
}
pub fn spl_init_invoke_signed<'info, A: Into<InitIxArgs>>(
    accounts: &InitAccounts<'_, 'info>,
    args: A,
    seeds: &[&[&[u8]]],
) -> ProgramResult {
