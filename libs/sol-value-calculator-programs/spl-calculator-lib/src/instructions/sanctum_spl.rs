...
const SVC_PROGRAM_ID: Pubkey = crate::sanctum_spl_sol_val_calc_program::ID;

pub fn sanctum_spl_lst_to_sol_ix(
    keys: LstToSolKeys,
    args: LstToSolIxArgs,
...
pub fn sanctum_spl_set_manager_ix(keys: SetManagerKeys) -> std::io::Result<Instruction> {
    set_manager_ix_with_program_id(SVC_PROGRAM_ID, keys)
}
pub fn sanctum_spl_set_manager_invoke(accounts: SetManagerAccounts) -> ProgramResult {
    set_manager_invoke_with_program_id(SVC_PROGRAM_ID, accounts)
}
pub fn sanctum_spl_set_manager_invoke_signed(
...
pub fn sanctum_spl_init_ix(keys: InitKeys) -> std::io::Result<Instruction> {
    init_ix_with_program_id(SVC_PROGRAM_ID, keys)
}
pub fn sanctum_spl_init_invoke(accounts: InitAccounts) -> ProgramResult {
    init_invoke_with_program_id(SVC_PROGRAM_ID, accounts)
}
pub fn sanctum_spl_init_invoke_signed(accounts: InitAccounts, seeds: &[&[&[u8]]]) -> ProgramResult {
