...
pub fn marinade_update_last_upgrade_slot_ix<K: Into<UpdateLastUpgradeSlotKeys>>(
    accounts: K,
) -> std::io::Result<Instruction> {
...
pub fn marinade_update_last_upgrade_slot_invoke<'info>(
    accounts: UpdateLastUpgradeSlotAccounts<'_, 'info>,
) -> ProgramResult {
    let ix = marinade_update_last_upgrade_slot_ix(accounts)?;
    let account_info: [AccountInfo<'info>; UPDATE_LAST_UPGRADE_SLOT_IX_ACCOUNTS_LEN] =
        accounts.into();
    invoke(&ix, &account_info)
}
pub fn marinade_update_last_upgrade_slot_invoke_signed<'info>(
    accounts: UpdateLastUpgradeSlotAccounts<'_, 'info>,
    seeds: &[&[&[u8]]],
) -> ProgramResult {
    let ix = marinade_update_last_upgrade_slot_ix(accounts)?;
...
pub fn marinade_set_manager_ix<K: Into<SetManagerKeys>>(
    accounts: K,
) -> std::io::Result<Instruction> {
...
pub fn marinade_set_manager_invoke<'info>(
    accounts: SetManagerAccounts<'_, 'info>,
) -> ProgramResult {
    let ix = marinade_set_manager_ix(accounts)?;
    let account_info: [AccountInfo<'info>; SET_MANAGER_IX_ACCOUNTS_LEN] = accounts.into();
    invoke(&ix, &account_info)
}
pub fn marinade_set_manager_invoke_signed<'info>(
    accounts: SetManagerAccounts<'_, 'info>,
    seeds: &[&[&[u8]]],
) -> ProgramResult {
    let ix = marinade_set_manager_ix(accounts)?;
...
pub fn marinade_init_ix<K: Into<InitKeys>>(accounts: K) -> std::io::Result<Instruction> {
    init_ix(accounts).map(|mut ix| {
        ix.program_id = crate::program::ID;
        ix
    })
}
pub fn marinade_init_invoke<'info>(accounts: InitAccounts<'_, 'info>) -> ProgramResult {
    let ix = marinade_init_ix(accounts)?;
    let account_info: [AccountInfo<'info>; INIT_IX_ACCOUNTS_LEN] = accounts.into();
    invoke(&ix, &account_info)
}
pub fn marinade_init_invoke_signed<'info>(
