...
pub fn marinade_update_last_upgrade_slot_invoke(
...
pub fn marinade_set_manager_ix(keys: SetManagerKeys) -> std::io::Result<Instruction> {
    set_manager_ix_with_program_id(crate::program::ID, keys)
}
pub fn marinade_set_manager_invoke(accounts: SetManagerAccounts) -> ProgramResult {
    set_manager_invoke_with_program_id(crate::program::ID, accounts)
}
pub fn marinade_set_manager_invoke_signed(
...
pub fn marinade_init_ix(keys: InitKeys) -> std::io::Result<Instruction> {
    init_ix_with_program_id(crate::program::ID, keys)
}
pub fn marinade_init_invoke(accounts: InitAccounts) -> ProgramResult {
    init_invoke_with_program_id(crate::program::ID, accounts)
}
pub fn marinade_init_invoke_signed(accounts: InitAccounts, seeds: &[&[&[u8]]]) -> ProgramResult {
